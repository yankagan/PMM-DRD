function [theta_opt, obj_hist, time_elapsed, acc_hist] = nonconvex_svm_mSGD_RMSprop(X, y, ...
...
%
% acc_hist  The history of classification accuracy following each training 
%           iteration
%
...
% RMSprop (ADAgrad with convex combination)
%
% 11/19/15
...
% The RMSprop initial gradient history
if ~isfield(options, 'rmsprop_h_init') || isempty(options.rmsprop_h_init)   
    options.rmsprop_h_init = 1e-2;
end

% The RMSprop learning rate
if ~isfield(options, 'rmsprop_learning_rate') || isempty(options.rmsprop_learning_rate)    
    options.rmsprop_learning_rate = 1e-2;
end

% The RMSprop convex combination weight (for the historical gradient)
if ~isfield(options, 'rmsprop_weight') || isempty(options.rmsprop_weight)    
    options.rmsprop_weight = 0.95;
end
...
grad_hist = options.rmsprop_h_init * ones(p, 1);
for k = 1: options.outer_max_iter
    ...
    grad_hist = options.rmsprop_weight * grad_hist + (1 - options.rmsprop_weight) * grad_k.^2;
    
    id_k = grad_k ./ sqrt(grad_hist);        
    
    step_k = - options.rmsprop_learning_rate * id_k;
...
acc_hist = acc_hist(1: k);

% Correct for NaN that shows up due to an early tol_fun, tol_grad, or 
% tol_step break
if isnan(acc_hist(end))
    obj_hist = obj_hist(1: end - 1);
    time_elapsed = time_elapsed(1: end - 1);
    acc_hist = acc_hist(1: end - 1);
end
