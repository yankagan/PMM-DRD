...
/*
PMM_CG_nonconvex_svm.cpp  

Usage:
	theta_out = PMM_CG_nonconvex_svm(Xy_grp, theta, grp_splits, cr_end_pt_info, l_vec, d_vec, lambda, inv_rho_grp, bias_grp, bias_idx, sc, options, num_outer_threads, num_inner_threads)
...
Inputs: 
	...
	cr_end_pt_info	A matrix with 8 columns containing the lower endpoint, upper endpoint, function value at both endpoints, first derivative at both endpoints, 
					and second derivative at both endpoints of the convexified projection 1D functions. The end points define a local convex region in the projected space
					It has dimensions: I x 8 (stored columnwise)

	l_vec			The previous projections. l_vec = Xy * theta. It has I elements
	
	d_vec			The negative of the minimum curvature in each projected local convex region (PLCR). It has I elements
	...
	sc				A scale parameter for the nonconvex SVM objective. A scalar
...
// A is a sparse matrix and b is a dense vector. c = A * b is the output vector
...
// A is a sparse matrix and b is a dense vector. c = b * A is the output vector
...
static void dense_vector_sparse_matrix_product(double* c, const mxArray *A, const double* b, const int n_threads = 0)
{
	...
	if (!mxIsSparse(A))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dense_vector_sparse_matrix_product:sparsity_mismatch", "A must be sparse");
...
namespace Convexified1D
{
	...
	*/
	...
	public:
		/** Constructor
			\arg sc_ is the scale parameter
		*/
		Function1D(type sc_) : sc(sc_)
		{
		}
		...
		*/
		...
		{
			return 1 - tanh(sc * v) - 0.5 * d * pow(v - l, 2);
		}

		/** Setter
			\arg sc_ is the scale parameter
		*/
		void set_variables(type sc_)
		{
			sc = sc_;
		}
	};
	...
	public:
		...
		*/
		type operator()(type v, type l, type d) const
		{
			return -sc * pow(1 / cosh(sc * v), 2) - d * (v - l);
		}
	...
	public:
		...
		*/
		...
		{
			return 2 * sc * sc * pow(1/cosh(sc * v), 2) * tanh(sc * v) - d;
		}
...
/** A function for computing the objective function based on the quadratic extension
	...
	\arg sc is the scale parameter of the loss function
	\arg d_vec is the minimum (local range) second derivative for each 1D projection
	\arg lambda is the regularization coefficient
	\arg cr_end_pt_info is an I x 8 matrix with end-point information for each of the I 1D projections
...
*/
static void obj_fun_param_split(double &obj_fun, double *l_vec, double *l_vec_hat, const double *theta, const mxArray *Xy, const double *theta_hat, const double sc,
	const double *d_vec, const double lambda, const double *cr_end_pt_info, const int bias_term, const double *l_vec_old, const double *inv_rho_loc, const int n_threads = 0)
{
	// Initialize the objective function
	obj_fun = 0;

	// Initialize the functor for the 1D objective function
	Convexified1D::Function1D<double> func(sc);
	...
	const double *l_lb = &cr_end_pt_info[0];
	const double *l_ub = &cr_end_pt_info[I];
	const double *f_con_lb = &cr_end_pt_info[2*I];
	const double *f_con_ub = &cr_end_pt_info[3 * I];
	const double *f_con_der_lb = &cr_end_pt_info[4 * I];
	const double *f_con_der_ub = &cr_end_pt_info[5 * I];
	const double *f_con_2der_lb = &cr_end_pt_info[6 * I];
	const double *f_con_2der_ub = &cr_end_pt_info[7 * I];

	// Compute data-fit part of the objective function	
	if (n_threads > 0)
	{
		#pragma omp parallel for default(shared) num_threads(n_threads)
		for (int i = 0; i < I; i++)
		{
			...
			if (rho > 0)
			{
				double obj_fun_part, l_diff;

				const double d_i = d_vec[i];
				const double l_lb_i = l_lb[i];
				const double l_ub_i = l_ub[i];
				const double f_con_lb_i = f_con_lb[i];
				const double f_con_ub_i = f_con_ub[i];
				const double f_con_der_lb_i = f_con_der_lb[i];
				const double f_con_der_ub_i = f_con_der_ub[i];
				const double f_con_2der_lb_i = f_con_2der_lb[i];
				const double f_con_2der_ub_i = f_con_2der_ub[i];
				...
				if (l_grp <= l_lb_i)
				{
					...
					obj_fun_part = f_con_lb_i + f_con_der_lb_i * l_diff + 0.5 * f_con_2der_lb_i * (l_diff * l_diff);
				}
				else if (l_grp >= l_ub_i)
				{
					// Quadratic extension
					l_diff = l_grp - l_ub_i;

					obj_fun_part = f_con_ub_i + f_con_der_ub_i * l_diff + 0.5 * f_con_2der_ub_i * (l_diff * l_diff);
				}
				else
				{
					// Convexified 1D objective
					obj_fun_part = func(l_grp, l_old, d_i);
				}

				// Incorporate the convexity factor
				obj_fun_part = rho * obj_fun_part;

				// Add to the objective value
				#pragma omp atomic 
				obj_fun += obj_fun_part;
			}
		}
	}
	else
	...
	if (n_threads > 0)
	{
		...
		for (int j = 0; j < J; j++)
		{
			...
			{
				#pragma omp atomic
				obj_fun += 0.5 * lambda * theta_j * theta_j;
			}
		}
	}
	else
...
*/
static void grad_fun_param_split(double *grad, double *l_vec, double *l_vec_hat, double *grad_fun_part, const double *theta, const mxArray *Xy, const double *theta_hat, const double sc,
	const double *d_vec, const double lambda, const double *cr_end_pt_info, const int bias_term, const double *l_vec_old, const double *inv_rho_loc, const int n_threads = 0)
{
	// Initialize the functor for the 1D derivative function
	Convexified1D::Gradient1D<double> func(sc);
	...
	// Compute data-fit part of the gradient function	
	if (n_threads > 0)
	{
		#pragma omp parallel for default(shared) num_threads(n_threads)
		for (int i = 0; i < I; i++)
		{
			...
			if (rho > 0)
			{
				...
				if (l_grp <= l_lb_i)
				{
					...
					grad_fun_part[i] = f_con_der_lb_i + f_con_2der_lb_i * l_diff;
				}
				else if (l_grp >= l_ub_i)
				{
					// Quadratic extension
					...
					grad_fun_part[i] = f_con_der_ub_i + f_con_2der_ub_i * l_diff;
				}
				else
				{
					// Convexified 1D first derivative
					grad_fun_part[i] = func(l_grp, l_old, d_i);
				}
			}
		}
	}
...
*/
static void hessian_vector_mult_fun_param_split(double *hess_vec, double *l_vec, double *l_vec_hat, double *hess_fun_part, double *xy_v, const double *theta, const double *v, const mxArray *Xy, const double *theta_hat, const double sc,
	const double *d_vec, const double lambda, const double *cr_end_pt_info, const int bias_term, const double *l_vec_old, const double *inv_rho_loc, const int n_threads = 0)
{
	// Initialize the functor for the 1D second derivative function
	Convexified1D::Hessian1D<double> func(sc);
	...
	// Compute data-fit part of the hessian-vector product	
	if (n_threads > 0)
	{
		#pragma omp parallel for default(shared) num_threads(n_threads)
		for (int i = 0; i < I; i++)
		{
			...
			if (rho > 0)
			{
				...
				if (l_grp <= l_lb_i)
				{
					hess_fun_part[i] = f_con_2der_lb_i;
				}
				else if (l_grp >= l_ub_i)
				{
					hess_fun_part[i] = f_con_2der_ub_i;
				}
				else
				{
					// Convexified 1D second derivative
					hess_fun_part[i] = func(l_grp, d_i);
				}

				// Incorporate the convexity factor
				hess_fun_part[i] *= inv_rho;
			}
		}
	}
	else
...
static void conjugate_gradient(double *s_opt, double *l_v, double *l_v_hat, double *hess_fun_part, double *xy_v, double *r_i, double *s_i, double *d_i, double *hess_times_d_i, double *s_i_plus_1, 
	...
	const double *d_vec, const double *cr_end_pt_info, const double *inv_rho_loc, const double lambda, const int bias_term, const double sc, 
	const Options opts, const double rad, const int J, const int n_threads = 0)
{
	if (n_threads > 0)
	{
		#pragma omp parallel for default(shared) num_threads(n_threads)
	...
	for (int i = 0; i < opts.cg_max_iter; i++)
	{
		...
		hessian_vector_mult_fun_param_split(hess_times_d_i, l_v, l_v_hat, hess_fun_part, xy_v, x_opt, d_i, A, x_prev, sc, d_vec, lambda, cr_end_pt_info, bias_term, l_vec, inv_rho_loc, n_threads);
		...
		if (l2_norm(s_i_plus_1, J) >= rad)
		{
			...
			{
				mexPrintf("a = %f, b = %f, c = %f, rt_1 = %f, rt_2 = %f, tau = %f\n", a, b, c, rt_1, rt_2, tau);
...
static void trust_region_ND_CG_unbounded(double *x_opt, double *l_v, double *l_v_hat, double *grad_fun_part, double *hess_fun_part, double *xy_v, 
	...
	const double *x_prev, const mxArray *A, const double *l_vec, const double *d_vec, const double *cr_end_pt_info,
	const double *inv_rho_loc, const double lambda, const int bias_term, const double sc, const Options opts, const int J, const int n_threads = 0)
{
	...
	grad_fun_param_split(grad, l_v, l_v_hat, grad_fun_part, x_opt, A, x_prev, sc, d_vec, lambda, cr_end_pt_info, bias_term, l_vec, inv_rho_loc, n_threads);
	...
	double fun_val_k;
	obj_fun_param_split(fun_val_k, l_v, l_v_hat, x_opt, A, x_prev, sc, d_vec, lambda, cr_end_pt_info, bias_term, l_vec, inv_rho_loc, n_threads);
	...
	for (int k = 0; k < opts.max_iter; k++)
	{
		...
		conjugate_gradient(s_opt, l_v, l_v_hat, hess_fun_part, xy_v, r_i, s_i, d_i, hess_times_d_i, s_i_plus_1, x_opt, grad, A, x_prev, l_vec, d_vec, cr_end_pt_info, 
			inv_rho_loc, lambda, bias_term, sc, opts, rad, J, n_threads);
		...
		obj_fun_param_split(new_fun_val_k, l_v, l_v_hat, x_new, A, x_prev, sc, d_vec, lambda, cr_end_pt_info, bias_term, l_vec, inv_rho_loc, n_threads);
		act_red_k = new_fun_val_k - fun_val_k;
		hessian_vector_mult_fun_param_split(hv, l_v, l_v_hat, hess_fun_part, xy_v, x_opt, s_opt, A, x_prev, sc, d_vec, lambda, cr_end_pt_info, bias_term, l_vec, inv_rho_loc, n_threads);
...
// 1 output, 17 inputs. Wow!
static void PMM_CG_nonconvex_svm(double *theta_out, const mxArray *Xy_grp, const double *theta, const double *grp_splits, const double *cr_end_pt_info, const double *l_vec,
						const double *d_vec, const double lambda, const double *inv_rho_grp, const int bias_grp, const int bias_idx, const double sc, 
						const mxArray *options, const int K, const int I, const int num_outer_threads, const int num_inner_threads)
{
	...
	// The fields are: Delta_k, eta_0, eta_1, eta_2, sigma_1, sigma_2, sigma_3, max_iter, fun_tol, grad_tol, rad_tol, cg_xi_k, cg_max_iter, display
	Options opts;
	...
	if (num_outer_threads > 0)
	{
		...
		for (int k = 0; k < K; ++k)
		{
			...
			trust_region_ND_CG_unbounded(theta_out_loc, l_v_loc, l_v_hat_loc, grad_fun_part_loc, hess_fun_part_loc, xy_v_loc, r_i_loc,
				...
				theta_loc, Xy_loc, l_vec, d_vec, cr_end_pt_info, inv_rho_loc, lambda, bias_term, sc, opts, J, num_inner_threads);
		}
	}
	else
	{
...
{	
	...
	if(nlhs > 1)
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:nargout", "PMM_CG_nonconvex_svm requires at most 1 output argument."); 

	if(nrhs < 12 || nrhs > 14)
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:nargin", "PMM_CG_nonconvex_svm requires at least 12 input arguments and at most 14.");
	
	// Check for input data type mismatch
	if (!IS_CELL_ARRAY_OF_REAL_MATRIX(prhs[0]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input Xy_grp must be a cell array of matrices.");

	if (!IS_REAL_1D_FULL_DOUBLE(prhs[1]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input theta must be a real, full, vector of doubles.");
	
	if (!IS_REAL_1D_FULL_DOUBLE(prhs[2]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input grp_splits must be a real, full, vector of doubles.");

	if (!IS_REAL_2D_FULL_DOUBLE(prhs[3]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input cr_end_pt_info must be a real, full, 2D matrix of doubles.");
	
	if (!IS_REAL_1D_FULL_DOUBLE(prhs[4]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input l_vec must be a real, full, vector of doubles.");

	if (!IS_REAL_1D_FULL_DOUBLE(prhs[5]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input d_vec must be a real, full, vector of doubles.");

	if (!IS_REAL_SCALAR_DOUBLE(prhs[6]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input lambda must be a scalar double.");

	if (!IS_REAL_2D_FULL_DOUBLE(prhs[7]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input inv_rho_grp must be a real, full, 2D matrix of doubles.");

	if (!IS_REAL_SCALAR_DOUBLE(prhs[8]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input bias_grp must be a scalar double.");
	
	if (!IS_REAL_SCALAR_DOUBLE(prhs[9]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input bias_idx must be a scalar double.");

	if (!IS_REAL_SCALAR_DOUBLE(prhs[10]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input sc must be a scalar double.");
		
	if (!IS_STRUCT(prhs[11]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input options must be a structure.");
	
	if(nrhs > 12)
	{
		if (!IS_REAL_SCALAR_DOUBLE(prhs[12]))
			mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input num_outer_threads must be a scalar double.");

		if (nrhs > 13)
		{
			if (!IS_REAL_SCALAR_DOUBLE(prhs[13]))
				mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:type_mismatch", "Input num_inner_threads must be a scalar double.");
		}
	}
	...
	if (K + 1 != mxGetNumberOfElements(prhs[2]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of partitions + 1 must match the number of elements in grp_splits.");

	if (I != mxGetM(prhs[3]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of rows in the matrix of each cell of Xy_grp must match the number of rows in cr_end_pt_info.");

	if (8 != mxGetN(prhs[3]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of columns in cr_end_pt_info must be 8.");

	if (I != mxGetNumberOfElements(prhs[4]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of rows in the matrix of each cell of Xy_grp must match the number of elements in l_vec.");

	if (I != mxGetNumberOfElements(prhs[5]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of rows in the matrix of each cell of Xy_grp must match the number of elements in d_vec.");

	if (I != mxGetM(prhs[7]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of rows in the matrix of each cell of Xy_grp must match the number of rows in inv_rho_grp.");

	if (K != mxGetN(prhs[7]))
		mexErrMsgIdAndTxt("MATLAB:PMM_CG_nonconvex_svm:dimension_mismatch", "The number of partitions must match the number of columns in inv_rho_grp.");
	...
	const double sc = mxGetScalar(prhs[10]);
	...
	PMM_CG_nonconvex_svm(theta_out, prhs[0], theta, grp_splits, cr_end_pt_info, l_vec, d_vec, lambda,  
		inv_rho_grp, bias_grp, bias_idx, sc, prhs[11], K, I, num_outer_threads, num_inner_threads);
