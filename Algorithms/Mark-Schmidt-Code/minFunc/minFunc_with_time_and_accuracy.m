function [x, f, exitflag, output] = minFunc_with_time_and_accuracy(funObj, x0, options, X_te, y_te, varargin)
% [x, f, exitflag, output] = minFunc_with_time_and_accuracy(funObj, x0, options, X_te, y_te, varargin)
%
...
%   stepTol - Termination tolerance on progress in terms of parameter changes (1e-9) Added 11/19/15
...
%
%   max_train_time - the maximum training time
%   n_display - How often to display objective function summary information
%
...
%
% Modified: 11/19/15
% To keep track of test classification accuracy for each iteration and to
% stop the algorithm when maximum training time is exceeded
...
% Get Parameters
% Modified 11/19/15 to include stepTol
[verbose,verboseI,debug,doPlot,maxFunEvals,maxIter,optTol,progTol,stepTol,method,...
...
if optCond <= optTol
    ...
    % Modified Modified to include timing information
    % 05/31/15

    if nargout > 3
        output = struct('iterations',0,'funcCount',1,...
            'algorithm',method,'firstorderopt',max(abs(g)),'message',msg,'trace',trace, 'timeElapsed', time_elapsed, 'accuracy', acc);
    end
    return;
end
...
for i = 1: maxIter
    ...
    switch method
        ...
        case TENSOR % Tensor Method
            ...
            options_sub.stepTol = stepTol; % Added 11/19/15
            options_sub.optTol = optTol;
            d = minFunc_with_time_and_accuracy(@taylorModel,zeros(p,1),options_sub,f,g,H,T);
...
%         return;
    ...
    % Modified 10/07/15
    if nargout > 3
        time_elapsed(end + 1, 1) = toc;
    end
    
    % Modified 10/07/15       
    if nargout > 3 && test_sample
        % Linear classification
        acc(end + 1, 1) = linear_classification_accuracy(X_te, y_te, x);
    end
    ...
    if verboseI && mod(i, options.n_display) == 0 % Modified 10/07/15
    ...
    if norm(t*d) <= stepTol % Modified 11/19/15
        exitflag=2;
        msg = 'Step Size below stepTol';
        break;
    end
...
%     end
    ...
    % Modified 10/07/15    
    if sum(time_elapsed(1: i)) >= options.max_train_time
        exitflag = 3;
        msg = 'Reached Maximum Number of Iterations';
        break;
    end
...
if nargout > 3
    % Added timeElapsed field to output structure
    % 05/31/15
    ...
    if test_sample
        ...
        output = struct('iterations',i,'funcCount',funEvals*funEvalMultiplier,...
            'algorithm',method,'firstorderopt',max(abs(g)),'message',msg,'trace',trace, 'timeElapsed', time_elapsed);
    end
end
